...
export default function UploadBox({ onFiles }) {
  ...
  const process = (fileList) => {
    ...
    const invalid = files.find(
      (f) => !ACCEPT.includes(f.type) || f.size > MAX_MB * 1024 * 1024
    );

    if (invalid) {
      setError("Only JPG/JPEG/PNG/PDF files up to 20 MB are allowed.");
      return;
    }
    setError("");
    onFiles(files);
  ...
  return (
    ...
    >
      ...
      <small>JPG, JPEG, PNG or PDF • Max 20 MB per file • Multiple files supported</small>
